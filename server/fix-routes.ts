...
export async function registerRoutes(app: Express) {
  ...
  api.get('/api/user', wrapAsync(requireAuth), wrapAsync(async (req: AuthenticatedRequest, res: Response) => {
    try {
      const user = await storage.getUserByEmail(String(req.user?.email || ''));
  ...
  api.patch('/api/user', wrapAsync(requireAuth), wrapAsync(async (req: AuthenticatedRequest, res: Response) => {
    try {
      const user = await storage.getUserByEmail(String(req.user?.email || ''));
      if (!user) return res.status(404).json({ error: 'User not found' });
      const allowedFields = ['firstName', 'lastName', 'phone', 'address', 'city', 'state', 'country', 'postalCode', 'profession', 'employer', 'annualIncome', 'avatarUrl', 'profilePhoto'];
      const allowedUpdates: Record<string, unknown> = {};
      for (const key of Object.keys(req.body || {})) {
        if (allowedFields.includes(key)) allowedUpdates[key] = req.body[key];
      }
      if (Object.keys(allowedUpdates).length === 0) return res.status(400).json({ error: 'No valid fields to update' });
  ...
  api.put('/api/admin/customers/:id', requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const admin = await storage.getUserByEmail(String(req.user?.email || ''));
  ...
  api.post('/api/admin/tickets/:id/respond', requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { data: ticket } = await getAdminClient().from('support_tickets').select('admin_notes').eq('id', id).single();
      ...
      await getAdminClient().from('support_tickets').update({ admin_notes: newNotes, status: status || 'responded', updated_at: new Date().toISOString() }).eq('id', id);
      await getAdminClient().from('admin_actions').insert({ admin_id: req.user?.id || '', action: 'ticket_respond', target_id: id, details: { description: `Responded to support ticket ${id}`, response: responseText } });
  ...
  api.post('/api/auth/change-password', requireAuth, authRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const supabaseClient = getAdminClient();
      const { error: signInError } = await supabaseClient.auth.signInWithPassword({ email: String(req.user?.email || ''), password: currentPassword });
  ...
  api.post('/api/payment-requests/:id/pay', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data: request } = await getAdminClient().from('transactions').select('*').eq('id', req.params.id).eq('transaction_type', 'payment_request').single();
      ...
      const { data: userAccount } = await getAdminClient().from('accounts').select('id, balance').eq('user_id', String(req.user?.id || '')).eq('status', 'active').limit(1).single();
      ...
      const toUserId = (request as Record<string, unknown>).to_user_id as string | null;
      let recipientAccountId: string | null = null;
      if (toUserId) {
        const { data: recipientAccount } = await getAdminClient().from('accounts').select('id').eq('user_id', toUserId).eq('status', 'active').limit(1).maybeSingle();
        if (recipientAccount) recipientAccountId = (recipientAccount as Record<string, unknown>).id as string;
      }
      if (recipientAccountId) {
        const transferResult = await atomicTransfer({ fromAccountId: accountId, toAccountId: recipientAccountId, amount, transactionType: 'payment', description: `Payment for request ${req.params.id}` });
        if (!transferResult.success) return res.status(400).json({ error: transferResult.error || 'Transfer failed' });
      } else {
      ...
      const newBalance = (await getAdminClient().from('accounts').select('balance').eq('id', accountId).single()).data?.balance || '0';
      await getAdminClient().from('transactions').update({ status: 'completed', completed_at: new Date().toISOString(), from_user_id: String(req.user?.id || '') }).eq('id', req.params.id);
      await getAdminClient().from('transactions').insert({ from_user_id: String(req.user?.id || ''), to_user_id: toUserId, amount: amount.toFixed(2), currency: 'USD', transaction_type: 'payment', category: 'payment', status: 'completed', description: `Payment for request ${req.params.id}`, reference_number: `PAY${Date.now()}${cryptoRandomInt(0, 9999)}`, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() });
      await getAdminClient().from('alerts').insert({ user_id: String(req.user?.id || ''), title: 'Payment Sent', message: `Payment of ${amount.toFixed(2)} has been sent.`, type: 'success', priority: 'normal', is_read: false });
  ...
  api.post('/api/add-funds', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      try {
        const { data: account } = await getAdminClient().from('accounts').select('id, balance').eq('user_id', user.id).eq('status', 'active').limit(1).single();
        ...
        await getAdminClient().from('alerts').insert({ user_id: String(req.user?.id || ''), title: 'Funds Added', message: `${parsedAmount.toFixed(2)} has been added to your account via ${sanitizedMethod}.`, type: 'success', priority: 'normal', is_read: false });
  ...
  api.get('/api/recent-contacts', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('recent_contacts').select('*').eq('user_id', String(req.user?.id || '')).order('updated_at', { ascending: false }).limit(10);
  ...
  api.get('/api/loans', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('loans').select('*').eq('user_id', String(req.user?.id || '')).order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/loans/apply', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const loanNumber = `LN${Date.now()}${cryptoRandomInt(0, 9999)}`;
      const { data, error } = await getAdminClient().from('loans').insert({ user_id: String(req.user?.id || ''), loan_number: loanNumber, loan_type: loanType, principal_amount: String(principalAmount), interest_rate: String(interestRate), term_months: termMonths, monthly_payment: monthlyPayment.toFixed(2), remaining_balance: String(principalAmount), total_interest: totalInterest.toFixed(2), total_payable: totalPayable.toFixed(2), status: 'pending' }).select().single();
      if (error) throw error;
      const { data: admins } = await getAdminClient().from('users').select('id').eq('role', 'admin').eq('is_active', true);
      if (admins && admins.length > 0) { const adminAlerts = admins.map((admin: Record<string, unknown>) => ({ user_id: admin.id, title: 'New Loan Application', message: `Loan application for ${principalAmount} from ${req.user?.email} requires review.`, type: 'warning', priority: 'high', is_read: false })); await getAdminClient().from('alerts').insert(adminAlerts); }
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/loans/:id/approve', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data: loan, error: loanError } = await getAdminClient().from('loans').select('*').eq('id', req.params.id).single();
      ...
      const { data, error } = await getAdminClient().from('loans').update({ status: 'approved', approved_by: String(req.user?.id || ''), approved_at: new Date().toISOString(), disbursement_date: new Date().toISOString(), maturity_date: new Date(Date.now() + (loan.term_months * 30 * 24 * 60 * 60 * 1000)).toISOString() }).eq('id', req.params.id).eq('status', 'pending').select().single();
      if (error || !data) return res.status(409).json({ error: 'Loan was already processed by another admin' });
      const { data: account } = await getAdminClient().from('accounts').select('id, balance').eq('user_id', loan.user_id).eq('status', 'active').limit(1).single();
      if (account) {
        const accountId = (account as Record<string, unknown>).id as string;
        ...
        await getAdminClient().from('transactions').insert({ from_account_id: null, to_account_id: (account as Record<string, unknown>).id, from_user_id: null, to_user_id: loan.user_id, amount: principalAmount.toFixed(2), currency: 'USD', transaction_type: 'loan_disbursement', category: 'loan', status: 'completed', description: `Loan disbursement - ${loan.loan_type} - ${loan.loan_number}`, reference_number: `LOAN${Date.now()}${cryptoRandomInt(0, 9999)}`, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() });
        await getAdminClient().from('alerts').insert({ user_id: loan.user_id, title: 'Loan Approved', message: `Your ${loan.loan_type} loan of ${principalAmount.toFixed(2)} has been approved and disbursed to your account.`, type: 'success', priority: 'high', is_read: false });
      }
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/loans/:id/reject', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data: loan, error: loanError } = await getAdminClient().from('loans').select('status').eq('id', req.params.id).single();
      if (loanError || !loan) return res.status(404).json({ error: 'Loan not found' });
      if (loan.status !== 'pending') return res.status(400).json({ error: 'Loan is not in pending status' });
      const { data, error } = await getAdminClient().from('loans').update({ status: 'rejected' }).eq('id', req.params.id).select().single();
      if (error) throw error;
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/admin/pending-loans', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('loans').select('*').eq('status', 'pending').order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/transactions/:id/reverse', requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const reversalAmount = parseFloat(String(transaction.amount)) || 0;
      if (transaction.fromAccountId) { const refundResult = await atomicBalanceUpdate(String(transaction.fromAccountId), reversalAmount, `Reversal refund for txn #${txnId}`); if (!refundResult.success) return res.status(400).json({ error: refundResult.error || 'Failed to refund reversed transaction' }); }
      if (transaction.toAccountId) { const debitResult = await atomicBalanceUpdate(String(transaction.toAccountId), -reversalAmount, `Reversal debit for txn #${txnId}`); if (!debitResult.success) { await atomicBalanceUpdate(String(transaction.fromAccountId), -reversalAmount, `Reversal refund rollback for txn #${txnId}`); return res.status(400).json({ error: 'Failed to debit recipient during reversal' }); } }
      ...
      await storage.updateTransactionStatus(txnId, 'reversed', String(req.user?.id || ''), sanitizedReason);
  ...
  api.get('/api/cards', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('cards').select('*').eq('user_id', String(req.user?.id || '')).order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/cards/lock', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { data, error } = await getAdminClient().from('cards').update({ status: locked ? 'locked' : 'active', updated_at: new Date().toISOString() }).eq('id', cardId).eq('user_id', String(req.user?.id || '')).select().single();
      if (error) throw error;
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/cards/settings', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { data, error } = await getAdminClient().from('cards').update({ daily_limit: dailyLimit, monthly_limit: monthlyLimit, is_contactless: isContactless, updated_at: new Date().toISOString() }).eq('id', cardId).eq('user_id', String(req.user?.id || '')).select().single();
      if (error) throw error;
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/cards', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { data: account } = await getAdminClient().from('accounts').select('id').eq('user_id', String(req.user?.id || '')).eq('status', 'active').limit(1).single();
      if (!account) return res.status(404).json({ error: 'No active account found' });
      const cardNumber = '4' + cryptoRandomInt(1000000000000000, 9999999999999999).toString();
      const expiryMonth = cryptoRandomInt(1, 12);
      const expiryYear = new Date().getFullYear() + 4;
      const { data, error } = await getAdminClient().from('cards').insert({ user_id: String(req.user?.id || ''), account_id: (account as Record<string, unknown>).id, card_number: cardNumber, card_type: cardType, cardholder_name: cardholderName, expiry_month: expiryMonth, expiry_year: expiryYear, status: 'active', is_contactless: true, daily_limit: '5000.00', monthly_limit: '50000.00', pin_set: false }).select().single();
      if (error) throw error;
      await getAdminClient().from('alerts').insert({ user_id: String(req.user?.id || ''), title: 'New Card Created', message: `A new ${cardType} card has been created for your account.`, type: 'success', priority: 'normal', is_read: false });
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/alerts', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('alerts').select('*').eq('user_id', String(req.user?.id || '')).order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.patch('/api/alerts/:id/read', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('alerts').update({ is_read: true, read_at: new Date().toISOString() }).eq('id', req.params.id).eq('user_id', String(req.user?.id || '')).select().single();
      if (error) throw error;
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.delete('/api/alerts/:id', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { error } = await getAdminClient().from('alerts').delete().eq('id', req.params.id).eq('user_id', String(req.user?.id || ''));
      if (error) throw error;
  ...
  api.get('/api/investments', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('investments').select('*').eq('user_id', String(req.user?.id || '')).order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/market-rates', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('forex').select('*').order('to_currency', { ascending: true });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/currency-exchange', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { data: rate, error: rateError } = await getAdminClient().from('forex').select('rate').eq('to_currency', sanitizedToCurrency).maybeSingle();
      ...
      if (!userAccount) return res.status(404).json({ error: 'Account not found' });
      const accountId = (userAccount as Record<string, unknown>).id as string;
      const debitResult = await atomicBalanceUpdate(accountId, -numAmount, `Currency exchange: ${numAmount} ${sanitizedFromCurrency} to ${sanitizedToCurrency}`);
      if (!debitResult.success) {
        return res.status(400).json({ error: debitResult.error || 'Insufficient funds' });
      }
      const creditResult = await atomicBalanceUpdate(accountId, convertedAmount, `Currency exchange credit: ${sanitizedToCurrency}`);
      if (!creditResult.success) {
        await atomicBalanceUpdate(accountId, numAmount, `Currency exchange rollback: ${sanitizedFromCurrency}`);
        return res.status(500).json({ error: 'Currency exchange failed during credit' });
      }
      const reference = `EXC${Date.now()}${cryptoRandomInt(0, 9999)}`;
      const { data: txn, error: txnError } = await getAdminClient().from('transactions').insert({ from_account_id: null, to_account_id: null, from_user_id: String(req.user?.id || ''), amount: numAmount.toFixed(2), currency: sanitizedFromCurrency, exchange_rate: exchangeRate.toFixed(4), converted_amount: convertedAmount.toFixed(2), transaction_type: 'currency_exchange', category: 'exchange', status: 'completed', description: `Currency exchange: ${numAmount} ${sanitizedFromCurrency} to ${convertedAmount.toFixed(2)} ${sanitizedToCurrency}`, reference_number: reference, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() }).select().single();
  ...
  api.get('/api/support-tickets', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('support_tickets').select('*').eq('user_id', String(req.user?.id || '')).order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/support-tickets', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { data, error } = await getAdminClient().from('support_tickets').insert({ user_id: String(req.user?.id || ''), subject, description, priority: priority || 'normal', status: 'open' }).select().single();
      if (error) throw error;
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/admin/support-tickets', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('support_tickets').select('*').order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/admin/transactions', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('transactions').select('*').order('created_at', { ascending: false }).limit(100);
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/admin/transactions', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const reference = `ADM${Date.now()}${cryptoRandomInt(0, 9999)}`;
      const { data, error } = await getAdminClient().from('transactions').insert({ from_account_id: accountId, to_account_id: null, from_user_id: String(userId || req.user?.id || ''), amount: Number(amount).toFixed(2), transaction_type: type, category: 'admin', status: 'completed', description: description || 'Admin transaction', reference_number: reference, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() }).select().single();
      if (error) throw error;
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/savings', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('savings').select('*').eq('user_id', String(req.user?.id || '')).order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/savings', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (deposit > 0) { const { data: userAccount } = await supabaseClient.from('accounts').select('id').eq('user_id', String(req.user?.id || '')).eq('status', 'active').limit(1).maybeSingle(); if (!userAccount) return res.status(404).json({ error: 'Account not found' }); const accountId = (userAccount as Record<string, unknown>).id as string; const balanceResult = await atomicBalanceUpdate(accountId, -deposit, 'Initial savings deposit'); if (!balanceResult.success) return res.status(400).json({ error: balanceResult.error || 'Insufficient funds for initial deposit' }); }
      const savingsNumber = `SAV${Date.now()}${cryptoRandomInt(0, 9999)}`;
      const { data, error } = await supabaseClient.from('savings').insert({ user_id: String(req.user?.id || ''), savings_type: accountType || 'savings', current_amount: deposit.toFixed(2), goal_name: goalName || null, target_amount: targetAmount || null, interest_rate: '2.50', status: 'active' }).select().single();
      if (error) throw error;
      if (deposit > 0) { await supabaseClient.from('transactions').insert({ from_user_id: String(req.user?.id || ''), to_user_id: String(req.user?.id || ''), amount: deposit.toFixed(2), currency: 'USD', transaction_type: 'savings_deposit', category: 'savings', status: 'completed', description: `Initial deposit to savings account ${savingsNumber}`, reference_number: `SAV${Date.now()}${cryptoRandomInt(0, 9999)}`, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() }); }
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/savings/deposit', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (isNaN(numAmount) || numAmount <= 0) return res.status(400).json({ error: 'Amount must be greater than zero' });
      const supabaseClient = getAdminClient();
      const { data: userAccount } = await supabaseClient.from('accounts').select('id, balance').eq('user_id', String(req.user?.id || '')).eq('status', 'active').limit(1).single();
      if (!userAccount) return res.status(404).json({ error: 'Account not found' });
      const accountId = (userAccount as Record<string, unknown>).id as string;
      ...
      const { data: savings } = await supabaseClient.from('savings').select('current_amount').eq('id', savingsId).eq('user_id', String(req.user?.id || '')).single();
      if (!savings) { await atomicBalanceUpdate(accountId, numAmount, 'Savings deposit rollback'); return res.status(404).json({ error: 'Savings account not found' }); }
      const currentSavingsBalance = String((savings as Record<string, unknown>).current_amount || '0');
      const newSavingsBalance = (parseFloat(currentSavingsBalance) + numAmount).toFixed(2);
      const { data: updatedSavings, error: savingsUpdateError } = await supabaseClient.from('savings').update({ current_amount: newSavingsBalance, updated_at: new Date().toISOString() }).eq('id', savingsId).eq('current_amount', currentSavingsBalance).select().single();
      if (savingsUpdateError || !updatedSavings) {
        await atomicBalanceUpdate(accountId, numAmount, 'Savings deposit rollback');
        return res.status(409).json({ error: 'Savings balance was modified by another transaction. Please try again.' });
      }
      await supabaseClient.from('transactions').insert({ from_user_id: String(req.user?.id || ''), to_user_id: String(req.user?.id || ''), amount: numAmount.toFixed(2), currency: 'USD', transaction_type: 'savings_deposit', category: 'savings', status: 'completed', description: `Deposit to savings account`, reference_number: `SAV${Date.now()}${cryptoRandomInt(0, 9999)}`, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() });
  ...
  api.post('/api/savings/withdraw', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (userAccount) {
        const accountId = (userAccount as Record<string, unknown>).id as string;
        const creditResult = await atomicBalanceUpdate(accountId, numAmount, 'Withdrawal from savings account');
        if (!creditResult.success) {
          await supabaseClient.from('savings').update({ current_amount: savingsBalance.toFixed(2), updated_at: new Date().toISOString() }).eq('id', savingsId);
          return res.status(500).json({ error: 'Failed to credit account after savings withdrawal' });
        }
      }
      await supabaseClient.from('transactions').insert({ from_user_id: String(req.user?.id || ''), to_user_id: String(req.user?.id || ''), amount: numAmount.toFixed(2), currency: 'USD', transaction_type: 'savings_withdrawal', category: 'savings', status: 'completed', description: `Withdrawal from savings account`, reference_number: `SAW${Date.now()}${cryptoRandomInt(0, 9999)}`, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() });
      return res.json({ success: true, newSavingsBalance });
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.post('/api/investments/buy', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { data: existing } = await supabaseClient.from('investments').select('id, shares, average_price').eq('user_id', String(req.user?.id || '')).eq('symbol', sanitizedSymbol).limit(1);
      if (existing && existing.length > 0) { const existingShares = parseFloat(String((existing[0] as Record<string, unknown>).shares || '0')); const existingAvg = parseFloat(String((existing[0] as Record<string, unknown>).average_price || '0')); const newTotalShares = existingShares + numShares; const newAvgPrice = ((existingAvg * existingShares) + (numPrice * numShares)) / newTotalShares; const { error: updErr } = await supabaseClient.from('investments').update({ shares: newTotalShares.toString(), average_price: newAvgPrice.toFixed(2), current_price: numPrice.toFixed(2), updated_at: new Date().toISOString() }).eq('id', String((existing[0] as Record<string, unknown>).id)); if (updErr) investmentError = true; } else { const { error: insErr } = await supabaseClient.from('investments').insert({ user_id: String(req.user?.id || ''), symbol: sanitizedSymbol, asset_type: assetType || 'stock', shares: numShares.toString(), average_price: numPrice.toFixed(2), current_price: numPrice.toFixed(2), status: 'active' }); if (insErr) investmentError = true; }
      if (investmentError) { await atomicBalanceUpdate(accountId, totalCost, 'Investment buy rollback'); return res.status(500).json({ error: 'Failed to record investment' }); }
      await supabaseClient.from('transactions').insert({ from_user_id: String(req.user?.id || ''), amount: totalCost.toFixed(2), currency: 'USD', transaction_type: 'investment_buy', category: 'investment', status: 'completed', description: `Bought ${numShares} shares of ${sanitizedSymbol} at ${numPrice.toFixed(2)}`, reference_number: `INV${Date.now()}${cryptoRandomInt(0, 9999)}`, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() });
  ...
  api.post('/api/investments/sell', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const supabaseClient = getAdminClient();
      const { data: investment } = await supabaseClient.from('investments').select('id, shares, average_price, symbol').eq('id', investmentId).eq('user_id', String(req.user?.id || '')).single();
      ...
      if (userAccount) {
        const accountId = (userAccount as Record<string, unknown>).id as string;
        const creditResult = await atomicBalanceUpdate(accountId, totalProceeds, `Sold ${numShares} shares of ${(investment as Record<string, unknown>).symbol} at ${numPrice.toFixed(2)}`);
        if (!creditResult.success) {
          await supabaseClient.from('investments').update({ shares: heldShares.toString(), current_price: String((investment as Record<string, unknown>).average_price || '0'), updated_at: new Date().toISOString() }).eq('id', investmentId);
          return res.status(500).json({ error: 'Failed to credit account after investment sale' });
        }
      }
      const symbol = (investment as Record<string, unknown>).symbol as string;
      await supabaseClient.from('transactions').insert({ from_user_id: null, to_user_id: String(req.user?.id || ''), amount: totalProceeds.toFixed(2), currency: 'USD', transaction_type: 'investment_sell', category: 'investment', status: 'completed', description: `Sold ${numShares} shares of ${symbol} at ${numPrice.toFixed(2)}`, reference_number: `SEL${Date.now()}${cryptoRandomInt(0, 9999)}`, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() });
  ...
  api.get('/api/payments', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('payments').select('*').eq('user_id', String(req.user?.id || '')).order('created_at', { ascending: false });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/kyc/status', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('kyc').select('*').eq('user_id', String(req.user?.id || '')).limit(1).single();
      if (error && error.code !== 'PGRST116') throw error;
      const { data: user } = await getAdminClient().from('users').select('is_verified, kyc_status, email, phone_number, full_name, address, city, country, profession, annual_income').eq('id', String(req.user?.id || '')).single();
  ...
  api.post('/api/kyc/submit', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const supabaseClient = getAdminClient();
      const { data, error } = await supabaseClient.from('kyc').upsert({ user_id: String(req.user?.id || ''), document_type: documentType, document_number: documentNumber || null, full_name: fullName, date_of_birth: dateOfBirth || null, nationality: nationality || null, address: address || null, status: 'pending', submitted_at: new Date().toISOString() }).select().single();
      if (error) throw error;
      await supabaseClient.from('users').update({ kyc_status: 'in_review' }).eq('id', String(req.user?.id || ''));
      await supabaseClient.from('alerts').insert({ user_id: String(req.user?.id || ''), title: 'KYC Submitted', message: 'Your KYC documents have been submitted for review.', type: 'info', priority: 'normal', is_read: false });
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/user/preferences', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('users').select('notification_preferences, privacy_preferences, display_preferences, security_preferences').eq('id', String(req.user?.id || '')).single();
      if (error) throw error;
  ...
  api.put('/api/user/preferences', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { error } = await getAdminClient().from('users').update(updateData).eq('id', String(req.user?.id || ''));
      if (error) throw error;
      return res.json({ success: true });
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.put('/api/user/security-questions', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const hashedAnswer1 = await bcrypt.hash(String(securityAnswer1).toLowerCase().trim(), 10);
      const hashedAnswer2 = await bcrypt.hash(String(securityAnswer2).toLowerCase().trim(), 10);
      const { error } = await getAdminClient().from('users').update({ security_question_1: securityQuestion1, security_answer_1: hashedAnswer1, security_question_2: securityQuestion2, security_answer_2: hashedAnswer2 }).eq('id', String(req.user?.id || ''));
      if (error) throw error;
      return res.json({ success: true });
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/transactions/export', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data: accounts } = await getAdminClient().from('accounts').select('id').eq('user_id', String(req.user?.id || ''));
      ...
      const { data: transactions } = await getAdminClient().from('transactions').select('*').or(`from_account_id.in.(${accountIds.join(',')}),to_account_id.in.(${accountIds.join(',')})`).order('created_at', { ascending: false }).limit(1000);
  ...
  api.post('/api/loans/:id/repay', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const supabaseClient = getAdminClient();
      const { data: loan } = await supabaseClient.from('loans').select('*').eq('id', req.params.id).eq('user_id', String(req.user?.id || '')).single();
      ...
      const { data: account } = await supabaseClient.from('accounts').select('id, balance').eq('user_id', String(req.user?.id || '')).eq('status', 'active').limit(1).single();
      ...
      await supabaseClient.from('transactions').insert({ from_user_id: String(req.user?.id || ''), amount: numAmount.toFixed(2), currency: 'USD', transaction_type: 'loan_repayment', category: 'loan', status: 'completed', description: `Loan repayment for loan ${req.params.id}`, reference_number: `LRP${Date.now()}${cryptoRandomInt(0, 9999)}`, processed_at: new Date().toISOString(), completed_at: new Date().toISOString() });
      await supabaseClient.from('alerts').insert({ user_id: String(req.user?.id || ''), title: 'Loan Payment Made', message: `Payment of ${numAmount.toFixed(2)} applied to your loan. Remaining balance: ${remainingBalance.toFixed(2)}.`, type: 'success', priority: 'normal', is_read: false });
  ...
  api.get('/api/loans/:id', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('loans').select('*').eq('id', req.params.id).eq('user_id', String(req.user?.id || '')).single();
      if (error) throw error;
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/cards/:id', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data: account } = await getAdminClient().from('accounts').select('id').eq('user_id', String(req.user?.id || ''));
      if (!account) return res.status(404).json({ error: 'Account not found' });
      const { data, error } = await getAdminClient().from('cards').select('*').eq('id', req.params.id).in('account_id', account.map((a: Record<string, unknown>) => a.id)).single();
      if (error) throw error;
      return res.json(data);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/branches', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('branches').select('*').eq('is_active', true).order('name', { ascending: true });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/atms', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('atms').select('*').eq('is_active', true).order('name', { ascending: true });
      if (error) throw error;
      return res.json(data || []);
    } catch (error: unknown) { return res.status(500).json({ error: 'An internal error occurred' }); }
  });
  ...
  api.get('/api/exchange-rates/changes', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { data, error } = await getAdminClient().from('forex').select('to_currency, rate, updated_at');
      if (error) throw error;
  ...
  api.post('/api/auth/check-email', authRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
  ...
  api.post('/api/auth/register-complete', registrationRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
  ...
  api.post('/api/admin/transfers/:id/approve', requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { data, error } = await adminClient.from('transactions').update({
        status: 'completed',
        approved_by: String(req.user?.id || ''),
        approved_at: new Date().toISOString(),
        completed_at: new Date().toISOString(),
        admin_notes: notes || null,
        updated_at: new Date().toISOString(),
      }).eq('id', id).eq('status', 'pending').select().single();
      if (error || !data) return res.status(409).json({ error: 'Transaction was already processed' });

      const txnRecord = data as Record<string, unknown>;
      const fromAccId = txnRecord.from_account_id as string | null;
      const toAccId = txnRecord.to_account_id as string | null;
      const txnAmount = parseFloat(String(txnRecord.amount || '0'));
      if (fromAccId && toAccId) {
        const transferResult = await atomicTransfer({ fromAccountId: fromAccId, toAccountId: toAccId, amount: txnAmount, transactionType: String(txnRecord.transaction_type || 'transfer'), description: String(txnRecord.description || 'Approved transfer') });
        if (!transferResult.success) {
          await adminClient.from('transactions').update({ status: 'pending' }).eq('id', id);
          return res.status(400).json({ error: transferResult.error || 'Failed to execute transfer' });
        }
      } else if (toAccId) {
        const creditResult = await atomicBalanceUpdate(toAccId, txnAmount, `Approved transfer credit - ${id}`);
        if (!creditResult.success) {
          await adminClient.from('transactions').update({ status: 'pending' }).eq('id', id);
          return res.status(400).json({ error: 'Failed to credit recipient' });
        }
      } else if (fromAccId) {
        const debitResult = await atomicBalanceUpdate(fromAccId, -txnAmount, `Approved transfer debit - ${id}`);
        if (!debitResult.success) {
          await adminClient.from('transactions').update({ status: 'pending' }).eq('id', id);
          return res.status(400).json({ error: 'Failed to debit sender' });
        }
      }
  ...
  api.post('/api/international-transfers', requireAuth, transactionRateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const reference = `INT${Date.now()}${cryptoRandomInt(0, 9999)}`;
      try {
  ...
  api.post('/api/admin/create-transaction', requireAdmin, wrapAsync(async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const account = accounts[0];
      const ref = reference || `ADM${Date.now()}${cryptoRandomInt(0, 9999)}`;
  ...
  api.post('/api/admin/customers/:id/balance', requireAdmin, wrapAsync(async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const ref = `BAL${Date.now()}${cryptoRandomInt(0, 9999)}`;
      await getAdminClient().from('transactions').insert({
  ...
  api.post('/api/admin/accounts/:id/balance', requireAdmin, wrapAsync(async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const ref = `ACC${Date.now()}${cryptoRandomInt(0, 9999)}`;
      await getAdminClient().from('transactions').insert({
        from_account_id: numAmount >= 0 ? null : accountId,
        to_account_id: numAmount >= 0 ? accountId : null,
        from_user_id: null,
  ...
  api.post('/api/messages', requireAuth, wrapAsync(async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const sender = await storage.getUserByEmail(String(req.user?.email || ''));
      const senderName = sender ? `${sender.firstName || ''} ${sender.lastName || ''}`.trim() : String(req.user?.email || 'User');
...
export async function registerLiveChatRoutes(app: Express) {
  ...
  app.post('/api/chat/send', wrapAsync(requireAuth), wrapAsync(async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      try { const { data: adminUsers } = await getAdminClient().from('users').select('id').eq('role', 'admin').limit(1).single(); if (adminUsers?.id) adminUserId = adminUsers.id as string; } catch (error: unknown) { console.warn('Failed to query admin users:', error instanceof Error ? error.message : 'Unknown error'); }
      const { data: savedMsg, error } = await getAdminClient().from('messages').insert({ sender_id: user.id, sender_name: `${user.firstName} ${user.lastName}`.trim(), sender_role: 'customer', recipient_id: adminUserId, message: message.trim(), conversation_id: `session_${user.id}`, is_read: false, created_at: new Date().toISOString() }).select().single();
      ...
      const { data: admins } = await getAdminClient().from('users').select('id').eq('role', 'admin').eq('is_active', true);
      if (admins && admins.length > 0) { const adminAlerts = admins.map((admin: Record<string, unknown>) => ({ user_id: admin.id, title: 'New Chat Message', message: `New message from ${req.user?.email}`, type: 'info', priority: 'normal', is_read: false })); await getAdminClient().from('alerts').insert(adminAlerts); }
