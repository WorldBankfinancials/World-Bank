...
export class MemStorage implements IStorage {
  ...
  private async initializeData() {
    // Only initialize if no users exist to prevent profile reset
    if (this.users.size > 0) {
      console.log('Storage already initialized, skipping to prevent profile reset');
      return;
    }
    ...
    // Create pending registration users for testing approval workflow
    const pendingUser1 = await this.createUser({
