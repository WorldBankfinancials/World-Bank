 ...
 * accountId / userId fields are UUID strings — not integers.
 */
import { z } from 'zod';
...
export const registrationSchema = z.object({
  ...
  password:    z.string().min(8).regex(/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/, 'Password requires uppercase, lowercase, and number'),
