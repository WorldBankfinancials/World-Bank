...
export default function Transfer() {
  ...
  const [recipientDetails, setRecipientDetails] = useState({
    ...
    phoneNumber: "", email: "", bankName: "", bankAddress: "", swiftCode: "",
    iban: "", accountNumber: "", routingNumber: "", purpose: "", relationship: ""
  });
  ...
  const verifyPinAndTransfer = async () => {
    if (!userId) { setPinError("You must be signed in"); return; }
    ...
    try {
      ...
      setPinError(err.message || "Transfer failed");
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  if (!authUser) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-gray-600">Please sign in to view transfers.</div>
      </div>
    );
  }
            ...
            <Card className="text-center">
              <CardContent className="pt-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-6">
                  <div className="flex justify-between items-center mb-2"><span className="text-sm text-gray-600">Reference Number</span><span className="font-mono text-sm font-medium">{transferReference}</span></div>
                  <div className="flex justify-between items-center mb-2"><span className="text-sm text-gray-600">Status</span><span className="text-sm font-medium text-orange-600">Processing</span></div>
                  <div className="flex justify-between items-center"><span className="text-sm text-gray-600">Estimated Time</span><span className="text-sm font-medium">1-3 business days</span></div>
                </div>
      ...
      <div className="px-4 py-6 pb-20">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex space-x-2">
            <Badge className="bg-green-100 text-green-800"><Shield className="w-3 h-3 mr-1" />Secure</Badge>
            <Badge className="bg-blue-100 text-blue-800"><Globe className="w-3 h-3 mr-1" />190+ Countries</Badge>
          </div>
        </div>
        ...
        <Card className="mb-6">
          <CardHeader><CardTitle>International Transfer Details</CardTitle></CardHeader>
          <CardContent className="space-y-6">
            <div className="bg-blue-50 p-4 rounded-lg">
              <Label htmlFor="amount" className="text-lg font-semibold">Transfer Amount (USD)</Label>
              <Input id="amount" type="number" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} className="text-2xl font-bold text-center mt-2" />
            ...
            <Button onClick={handleTransfer} disabled={!amount || !recipientDetails.fullName || !recipientDetails.accountNumber || isProcessing} className="w-full bg-blue-600 text-white h-12 hover:bg-blue-700">
              {isProcessing ? <><Clock className="w-4 h-4 mr-2 animate-spin" />Processing Transfer...</> : <><Globe className="w-4 h-4 mr-2" />Send ${amount || "0.00"} Internationally</>}
            </Button>
              ...
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-sm text-gray-600">Please enter your 4-digit PIN to authorize this ${amount} transfer</p>
                  </div>
                  <div className="mb-4">
                    <input type="password" value={transferPin} onChange={(e) => setTransferPin(e.target.value.replace(/\D/g, '').slice(0, 4))} className="w-full text-center text-2xl tracking-widest p-4 border border-gray-300 rounded-lg" placeholder="****" maxLength={4} autoFocus />
                    {pinError && <p className="text-red-600 text-sm mt-2 text-center">{pinError}</p>}
                  </div>
                  <div className="flex space-x-3">
                    <Button variant="outline" onClick={() => { setShowPinVerification(false); setTransferPin(""); setPinError(""); }} className="flex-1" disabled={isProcessing}>Cancel</Button>
                    <Button onClick={verifyPinAndTransfer} disabled={transferPin.length !== 4 || isProcessing} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white">{isProcessing ? "Processing..." : "Confirm Transfer"}</Button>
                  </div>
                </div>
              </div>
            )}
              ...
              {recentContacts.map((contact, index) => (
                <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div><p className="font-medium">{contact.name}</p><p className="text-sm text-gray-600">{contact.account}</p></div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Last: {contact.lastAmount}</p>
                    <Button size="sm" variant="outline" onClick={() => setRecipientDetails(prev => ({ ...prev, fullName: contact.name }))}>Select</Button>
                  </div>
                </div>
              ))}
            </div>
