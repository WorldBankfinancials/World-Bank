import { setStorageItem, getStorageItem } from '@/lib/storage-utils';
...
export default function SimpleAdmin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStorageItem('adminToken', data.token);
      setStorageItem('adminUser', JSON.stringify(data.user));
  ...
  const fetchSupportTickets = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        queryClient.setQueryData<SupportTicket[]>(['support-tickets'], data);
      }
    } catch (error) {
