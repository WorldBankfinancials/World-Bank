        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {pendingRequests && Array.isArray(pendingRequests) && pendingRequests.length > 0 ? pendingRequests.map((request: Record<string, unknown>, index: number) => (
                <div key={`item-${index}`} className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium text-sm">{String(request.from || '')}</p>
                      <p className="text-xs text-gray-500">{String(request.time || '')}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-medium">{String(request.amount || '')}</p>
                    <Badge 
